...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<?= APP_URL ?>/admin/followups" class="<?= ($activePage ?? '') === 'followups' ? 'active' : '' ?>" style="display:flex;align-items:center;justify-content:space-between">
            ...
            <span id="fup-badge" style="background:#ef4444;color:#fff;border-radius:10px;padding:1px 7px;font-size:11px;font-weight:700;min-width:18px;text-align:center<?= $_notifOverdue > 0 ? '' : ';display:none' ?>"><?= $_notifOverdue ?></span>
        </a>
...
<script>
(function() {
    var badge     = document.getElementById('fup-badge');
    var baseTitle = document.title.replace(/^\(\d+\)\s*/, '');
    var endpoint  = '<?= APP_URL ?>/<?= ($_SESSION['user_role'] ?? '') === 'admin' ? 'admin' : 'agent' ?>/notif-count';

    function applyCount(overdue, soon) {
        var total = overdue + soon;
    ...
    function poll() {
        fetch(endpoint, {credentials: 'same-origin'})
            ...
            .then(function(d) { if (d) applyCount(d.overdue || 0, d.soon || 0); })
            .catch(function() {});
    }
